...
from assignment_4_aux import ICPVisualizer, load_point_cloud, view_point_cloud, quaternion_matrix, \
...
def transform_point_cloud(point_cloud, t, R):
    ...
    # ------------------------------------------------
    # FILL WITH YOUR CODE
    coord = point_cloud[:,0:3]
    color = point_cloud[:,3:6]
    P_ = np.zeros((len(point_cloud),6))
    P_[:,3:6] = color 
    coord_ = R@np.transpose(coord)+t.reshape((3,1)) 
    P_[:,0:3] = coord_.T
    transformed_point_cloud = P_  # TODO: Replace None with your result
    # ------------------------------------------------
...
def merge_point_clouds(point_clouds, camera_poses):
    ...
    for idx in range(len(P)):
        length.append(len(P[idx]))
    P_merged = np.zeros((np.sum(np.array(length)),6))
    jdx = 0
    for idx in range(len(P)):
        Pi = P[idx]
        Ci = C[idx]
        ti = Ci[0]
        Ri = Ci[1]
        P_merged[jdx:jdx+length[idx],:]= transform_point_cloud(Pi, ti, Ri)
        jdx = jdx+length[idx]
    merged_point_cloud = P_merged   # TODO: Replace None with your result
    # ------------------------------------------------
...
def find_closest_points(point_cloud_A, point_cloud_B):
    ...
    # ------------------------------------------------
    # FILL WITH YOUR CODE
    Cba = np.zeros(len(point_cloud_A))
    PA = point_cloud_A[:,0:3]
    PB = point_cloud_B[:,0:3]
    i = 0
    for PAi in PA:
        dist = []
        diff = PAi - PB
        ...
        dist.append(norm)
        Cba[i] = np.argmin(np.array(dist))
        i += 1

    closest_points_indxs = np.array(Cba) # TODO: Replace None with your result
    # ------------------------------------------------
...
def find_best_transform(point_cloud_A, point_cloud_B):
    ...
    PB = point_cloud_B[:,0:3]
    N = len(point_cloud_A)
    sumPA = np.sum(PA, axis = 0)
    sumPB = np.sum(PB, axis = 0)
    muA = sumPA/N
    muB = sumPB/N
    W = 0
    for idx in range(N):
        W += (PA[idx]-muA).reshape((3,1))@(PB[idx]-muB).reshape((1,3))
...
def icp_step(point_cloud_A, point_cloud_B, t_init, R_init):
    ...
    # Transform PCB with t_init and R_init
    pB = transform_point_cloud(point_cloud_B, t_init, R_init)
    # Find correspondences between PCA and new PCB
    correspondences = find_closest_points(point_cloud_A, pB)
    correspondences = correspondences.astype(np.int64)
    # Apply correspondences by making a new point cloud the same size as PCA, 
    # but at each ith row, replace it with the component of B at correspondence[i]
    pc = np.zeros_like(point_cloud_A)
    for idx in range(len(pB)):
        if idx < len(correspondences):
            pc[idx] = point_cloud_B[correspondences[idx]]
        else:
            break
    # Find best transform between PCA and new point cloud.
    t,R = find_best_transform(point_cloud_A, pc)
    # ------------------------------------------------
...
def filter_point_cloud(point_cloud):
    ...
    # In part 6, you can further assume that the M block will be inside the 
    # space delimited by the 4 columns. Therefore, you can assume that M 
    # will be within the [-0.48,0.48]x[-0.48,0.48] xy space. 
    # This may help your filtering step.
    PC = point_cloud
    '''
    idx = np.where((point_cloud[:,0]>-0.48) & (point_cloud[:,0]<0.48) 
                        & (point_cloud[:,1]>-0.48) & (point_cloud[:,1]<0.48))
    PC = point_cloud[idx]
    '''
    filtered_pc = []
    i = 25
    ...
    for pc in PC:
        for z in range(i,j):
            cond[z-i] = (pc[3:]==np.array([241, 241, z])/255).all()
        Cond = np.any(cond)
        if Cond:
            filtered_pc.append(pc)
    filtered_pc = np.array(filtered_pc) 
...
if __name__ == '__main__':
    ...
    path_to_files = 'D:/OneDrive - Umich/Fall 2020/ME 599/HW/4/Assignment 4' # TODO: Change the path to the directory containing your point cloud files
