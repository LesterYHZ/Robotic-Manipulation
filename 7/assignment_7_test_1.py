...
def grasp(block, theta_ = 0):
    ...
    command.append([x, y, z/2, theta, width])
    command.append([x, y, z/2, theta, 0.05])
    command.append([x, y, 0.2, theta, 0.05])
...
def place(block, theta_ = 0, target = np.array([0, 0, 0.03])):
    ...
    command = []
    command.append([x_f, y_f, 0.2, theta, 0.05])
    command.append([x_f, y_f, z_f, theta, 0.05])
    command.append([x_f, y_f, z_f, theta, 0.20])
    command.append([x_f, y_f, 0.2, theta, 0.20])
...
def stack():
    ...
    for loop in np.array([0,1,2,3]):
        ...
        if (OBJ[loop] != "box"):
                    # & (OBJ[loop] != "red"): 
            robot_command = []
            robot_command = push(block, 0.025)        
            env.robot_command(robot_command)
        ...
        # place block
        robot_command = []
        robot_command = place(block, theta_, target = np.array([0, 0, cent_height]))     
        env.robot_command(robot_command)
